<?php
	...
	$get_total_count_sql = "SELECT count(id) FROM post WHERE status='publish' AND title like '%$search_query%' ORDER BY publish_date DESC";
	$get_total_count = $conn->query($get_total_count_sql);
	$get_total_count = $get_total_count->fetch_row()[0];

	// var_dump($conn);
	$sql = "SELECT * FROM post WHERE status='publish' AND title like '%$search_query%' ORDER BY publish_date DESC LIMIT $limit OFFSET $offset";
